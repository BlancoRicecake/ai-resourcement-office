...
- MCP 클라이언트(Claude Desktop 등)는 `worker/mcp` 서버가 시작할 때 이 파일을 읽어
  성장 레이어(`~/.web-copy-analyzer/`)와 사용자 우선으로 병합한 뒤 `instructions`(MCP
...
아래 워크플로에 등장하는 툴 이름(`fetch_page`, `parse_sections`,
`readability_scorecard`, `diagnose_section`, `rewrite_section`, `compare_report`,
`save_persona`/`list_personas`/`get_persona`/`delete_persona`, `remember`,
`save_workflow`)는 MCP 도구명이자 CLI 서브커맨드명과 1:1 대응한다(CLI는
`fetch-page`처럼 kebab-case, `persona save`처럼 그룹화됨 — worker/harness/README.md
참고).
