...
> 런타임에 이 절차 자체가 바뀌거나(예: 특정 업종용 변형 순서) 반복 요청받는
> 절차가 새로 생기면, seed 절차인 이 파일을 고치지 말고 `save_workflow`로
> 성장 레이어(`~/.web-copy-analyzer/workflows/`)에 별도로 기록한다.

## 절차

1. **페르소나 확보** — `list_personas` → `get_persona`. 저장된 페르소나가
   없으면 `persona-interview.md` 절차로 먼저 정의하고 `save_persona`로
   저장한다. 페르소나 없이 다음 단계로 진행하지 않는다.
...
9. **자가 점검** — agent.md ③-8의 루브릭 R1~R7을 점검한다. 하나라도
   "아니오"면 5~6단계로 복귀한다.
10. **성장 트리거** — 사용자의 선호/교정 표현이나 리라이트 채택/거부가
    있으면 `remember`를 호출하고 무엇을 기억했는지 항상 표시한다.
