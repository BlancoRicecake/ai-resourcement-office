...
### 2.5 지식 노드 컨벤션 (엔진 없는 그래프 — 표준)

직무 누적이 커지면 "매 세션 전부-로드" 비용이 누적량에 비례해 커진다. 이를 **엔진(코드)
없이** 데이터 구조만으로 푼다. 자세한 설계는 `knowledge-arch.md`.

```txt
memory/
  MEMORY.md USER.md PROJECT.md DECISIONS.md   ← 항상 로드 (작게 유지)
  <JOB>.md                                    ← 작업 로그(인박스). 최근 항목 위주로 확인
  knowledge/
    INDEX.md                                  ← 노드 목록: 한 줄 = id · 제목 · 태그 · 링크
    <node-id>.md                              ← 노드: frontmatter + 본문 + [[위키링크]]
```

- **노드** = 일반화 가능한 지식 한 개념 한 파일. frontmatter(`id`(=파일명)·`title`·`tags`·
  `source`(seed|learned)·`confidence`·`created`) + 본문 + `[[다른-노드-id]]` 엣지.
- **INDEX.md** = 노드당 정확히 한 줄(`- [[id]] 제목 — 태그: …`). 이게 검색엔진의 대체물이다:
  에이전트는 세션 시작 시 **INDEX만** 읽고, 작업과 관련된 노드 파일만 골라 연 뒤
  `[[링크]]`를 따라 이웃을 확장한다. 노드를 추가/삭제하면 INDEX를 같이 갱신한다
  (업데이트 후보로 제안 — 자동확정 금지 규칙 동일).
- **승격 규칙**: `<JOB>.md`(인박스)의 항목에서 **일반화 가능한 교훈**이 확인되면 노드로
  승격하고 원 항목엔 `→ [[node-id]]` 표기를 남긴다. 반복 확인된 노드는 새 노드를 만들지
  말고 기존 노드의 `confidence`를 올린다(중복 병합, 모순은 사용자 확인).
- **로드 규칙(AGENTS.md에 명시)**: 표준 4파일 + `<JOB>.md`는 읽고, `knowledge/`는
  **INDEX.md만** 읽는다. 노드 본문은 필요한 것만 연다.
- 코드형 번들이 엔진을 얹으면 같은 노드를 `search_knowledge`로 자동 탐색할 뿐,
  데이터 형식은 동일하다(부록 A).
